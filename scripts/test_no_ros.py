...
Usage:
    py -3.11 scripts/test_no_ros.py
"""
...
HUMAN_FEEDBACK = (FeedbackType.SKIP, "motor")
# HUMAN_FEEDBACK = (FeedbackType.LOCK,  "load")
# HUMAN_FEEDBACK = (FeedbackType.EARLY, "load")
# HUMAN_FEEDBACK = (FeedbackType.LATE,  "load")
# HUMAN_FEEDBACK = (FeedbackType.SWAP,  "load", "bearing")

# one color per slot in all_positions (7 slots)
...
def _plot_iteration(exp, iteration, u_act_memory_before, human_feedback):
    ...
    # ── memory before/after ───────────────────────────────────────────
    # plot all bins (all_positions), label only active ones
    action_name_map = dict(zip(exp.input_positions, exp.action_names))

    for i, (bucket_idx, color) in enumerate(zip(exp.action_buckets, ACTION_COLORS)):
        pos   = exp.all_positions[i]
        label = action_name_map.get(pos, f"slot {pos}")
